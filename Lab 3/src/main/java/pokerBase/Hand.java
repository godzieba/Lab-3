	...
	private static ArrayList<Hand> ExplodeHands(ArrayList<Hand> Hands) {
		...
		for (int i = 0; i < 5; i++) {
			if (hand.getCardsInHand().get(i).geteSuit() == eSuit.JOKER) {
				ArrayList<Hand> Hands2 = new ArrayList<Hand>();
				for (Hand hand2 : Hands) {
					ArrayList<Hand> Hands3 = new ArrayList<Hand>();
					for (eSuit Suit : eSuit.values()) {
						for (eRank Rank : eRank.values()) {
							if ((Suit != eSuit.JOKER) && (Rank != eRank.JOKER)) {
								hand2.getCardsInHand().get(i).seteSuit(Suit);
								hand2.getCardsInHand().get(i).seteRank(Rank);
								Hands3.add(hand2);
							}
						} 
					}
					Hands2.addAll(Hands3);
				}
				Hands = Hands2;
			}
		}
